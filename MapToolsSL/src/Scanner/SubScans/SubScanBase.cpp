...
SubScanBase::SubScanBase(CameraBase& camera, bool *& projectorMask, Histogram &histThresholdRange, int iSubScan) : 
_projectorScanMask(projectorMask),
_histThrehsoldRange(histThresholdRange),
_iSubScan(iSubScan),
_cameraPixels(camera.getPixelsRef()),
...
{
	for (int i=0; i<camPixelCount; i++)
        if (_calibrateMax[i] < _cameraPixels[i])
            _calibrateMax[i] = _cameraPixels[i];
}

void SubScanBase::addCalibrationLow()
{
	for (int i=0; i<camPixelCount; i++)
        if (_calibrateMin[i] > _cameraPixels[i])
            _calibrateMin[i] = _cameraPixels[i];
}
...
void SubScanBase::calcCalibration()
{
	...
	int nPixels = _cameraPixels.getWidth() * _cameraPixels.getHeight();
...
void SubScanBase::addScanFrame(int iSubScanFrame)
{
	...
	for (int i=0; i<camPixelCount; i++)
		if (_calibrateThresholdMask[i])
		{
			...
			_subFrameDataPreview[i] = float(_subFrameData[i])/ float(1 << (nScanFramesPerInterleave-1)) * 255;
...
void SubScanBase::calcBinary()
{
    ...
    for (int i=0; i<camPixelCount; i++)
	{
		_cameraBinary[i] = _cameraPixels[i] > _calibrateThresholdMid[i];
